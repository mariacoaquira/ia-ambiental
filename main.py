...
ALLOWED_ORIGIN  = "https://ia-ambiental-ashen.vercel.app"
ALLOWED_HEADERS = "Content-Type, Authorization, X-Requested-With, Accept, Origin, multipart/form-data"
    ...
    async def dispatch(self, request: Request, call_next):
        if request.method == "POST":
            ...
            if content_length and int(content_length) > MAX_UPLOAD_SIZE:
                return JSONResponse(
                    status_code=413,
                    content={"error": f"Archivo demasiado grande. Máximo permitido: 500MB"},
...
@app.get("/api/iga/pdf/{nombre_archivo}")
async def servir_pdf(nombre_archivo: str):
    ...
    bucket = storage_client.bucket("asistente-ambiental")
    blob   = bucket.blob(f"input/{nombre_archivo}")
